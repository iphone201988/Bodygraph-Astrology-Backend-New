...
exports.motorToThroat = exports.getActivationFromDecimalDegrees = exports.decimalDegreesToDMS = exports.Gates = exports.harmonicGates = exports.definedCentersByChannel = exports.circuitryByChannel = exports.createBodygraph = void 0;
...
function createBodygraph(name, date, location) {
    ...
    var personalitySunLongitude = -1;
    console.log("sdsdsds", date.getUTCFullYear(), date.getUTCMonth() + 1, // Months are 1 higher in swisseph than their index (e.g. September has index of 8 and number of 9)
    date.getUTCDate() + 1, (date.getUTCHours() + (date.getUTCMinutes() / 60)));
    // Configure Swiss Epehemeris for Julian Day and begin collecting positions
    swisseph.swe_julday(date.getUTCFullYear(), date.getUTCMonth() + 1, // Months are 1 higher in swisseph than their index (e.g. September has index of 8 and number of 9)
    date.getUTCDate() + 1, (date.getUTCHours() + (date.getUTCMinutes() / 60)), swisseph.SE_GREG_CAL, function (julday_ut) {
        console.log("🚀 ~ createBodygraph ~ julday_ut:", julday_ut);
        swisseph.swe_calc_ut(julday_ut, swisseph.SE_SUN, flag, function (body) {
            ...
            var earthPosition = body.longitude += 180;
        ...
        swisseph.swe_calc_ut(julday_ut, swisseph.SE_TRUE_NODE, flag, function (body) {
            ...
            var southNodePosition = body.longitude += 180;
    ...
    var searchingSunLongitude;
    var offset;
    if (personalitySunLongitude > 87) { // If it's 88º or higher
        offset = 89;
        while (offset > 88) {
            swisseph.swe_julday(searchingDate.getUTCFullYear(), searchingDate.getUTCMonth() + 1, // Months are 1 higher in swisseph than their index (e.g. September has index of 8 and number of 9)
            searchingDate.getUTCDate(), (searchingDate.getUTCHours() + (searchingDate.getUTCMinutes() / 60)), swisseph.SE_GREG_CAL, function (julday_ut) {
                swisseph.swe_calc_ut(julday_ut, swisseph.SE_SUN, flag, function (body) {
    ...
    swisseph.swe_julday(designDate.getUTCFullYear(), designDate.getUTCMonth() + 1, // Months are 1 higher in swisseph than their index (e.g. September has index of 8 and number of 9)
    designDate.getUTCDate(), (designDate.getUTCHours() + (designDate.getUTCMinutes() / 60)), swisseph.SE_GREG_CAL, function (julday_ut) {
    ...
    for (i = 0; i < allActivations.length; i++) {
        var gate = allActivations[i].Gate;
        for (var n = 0; n < exports.harmonicGates[gate].length; n++) {
            if (allActivatedGates[exports.harmonicGates[gate][n]]) {
                bodygraph.channels.push(Math.min(exports.harmonicGates[gate][n], gate) + "-" + Math.max(exports.harmonicGates[gate][n], gate));
            }
        }
    }
    ...
    for (i = 0; i < bodygraph.channels.length; i++) { // e.g. "29-46", "9-52"
        ...
        for (n = 0; n < exports.definedCentersByChannel[channel].length; n++) {
            bodygraph.definedCenters.push(exports.definedCentersByChannel[channel][n]);
        }
    }
    ;
    ...
    if (!bodygraph.definedCenters || bodygraph.definedCenters.length === 0) {
        bodygraph.innerAuthority = "null";
        bodygraph.outerAuthority = "Lunar";
    }
    else {
        const authorityHierarchy = ["Solar Plexus", "Sacral", "Spleen", "Ego", "G"];
        let innerAuthorityFound = false;
        for (let i = 0; i < authorityHierarchy.length; i++) {
            if (bodygraph.definedCenters.includes(authorityHierarchy[i])) {
                ...
                bodygraph.innerAuthority = authorityMap[authorityHierarchy[i]];
                innerAuthorityFound = true;
                break;
            }
        }
        if (!innerAuthorityFound) {
            bodygraph.innerAuthority = "No Inner Authority";
            bodygraph.outerAuthority = "Sounding Board";
        }
        else {
            bodygraph.outerAuthority = "No Outer Authority";
        }
    }
    ...
    if (bodygraph.definedCenters.length == 0) {
        ...
        areasOfDefinition[0][exports.definedCentersByChannel[bodygraph.channels[0]][0]] = true;
        areasOfDefinition[0][exports.definedCentersByChannel[bodygraph.channels[0]][1]] = true;
        for (i = 1; i < bodygraph.channels.length; i++) {
            ...
            if (areasOfDefinition.length == 1) {
                ...
                if (areasOfDefinition[0][exports.definedCentersByChannel[bodygraph.channels[i]][0]] ||
                    areasOfDefinition[0][exports.definedCentersByChannel[bodygraph.channels[i]][1]]) {
                    areasOfDefinition[0][exports.definedCentersByChannel[bodygraph.channels[i]][0]] = true;
                    areasOfDefinition[0][exports.definedCentersByChannel[bodygraph.channels[i]][1]] = true;
                }
                else { // Split found!
                    console.log('Split found! Creating a new area of definition...');
                    areasOfDefinition.push({}); // Create a new area of definition
                    areasOfDefinition[1][exports.definedCentersByChannel[bodygraph.channels[i]][0]] = true;
                    areasOfDefinition[1][exports.definedCentersByChannel[bodygraph.channels[i]][1]] = true;
                }
            }
            else {
                let matchFound = false;
                for (n = 0; n < areasOfDefinition.length; n++) {
                    if (areasOfDefinition[n][exports.definedCentersByChannel[bodygraph.channels[i]][0]] ||
                        areasOfDefinition[n][exports.definedCentersByChannel[bodygraph.channels[i]][1]]) {
                        areasOfDefinition[n][exports.definedCentersByChannel[bodygraph.channels[i]][0]] = true;
                        areasOfDefinition[n][exports.definedCentersByChannel[bodygraph.channels[i]][1]] = true;
                        matchFound = true;
                        break;
                    }
                }
                if (!matchFound) {
                    areasOfDefinition.push({}); // Create a new area of definition
                    areasOfDefinition[areasOfDefinition.length - 1][exports.definedCentersByChannel[bodygraph.channels[i]][0]] = true;
                    areasOfDefinition[areasOfDefinition.length - 1][exports.definedCentersByChannel[bodygraph.channels[i]][1]] = true;
                }
            }
    ...
    for (i = 0; i < bodygraph.channels.length; i++) {
        bodygraph.circuitries.push(exports.circuitryByChannel[bodygraph.channels[i]]);
    }
...
exports.createBodygraph = createBodygraph;
var adjacentCenters = {
    "Head": {
        "Ajna": true
    },
...
exports.definedCentersByChannel = {
    "1-8": ["G", "Throat"],
    "2-14": ["Sacral", "G"],
    "3-60": ["Root", "Sacral"],
    "4-63": ["Head", "Ajna"],
...
function decimalDegreesToDMS(decimalDegrees) {
    ...
    var degrees = Math.floor(parseInt(decimalDegrees));
    var degreesFloat = parseFloat(decimalDegrees);
    var minutes = Math.floor((degreesFloat - degrees) * 60);
    var seconds = Math.round((degreesFloat - degrees - minutes / 60) * 3600);
    return {
        'degrees': degrees,
        'minutes': minutes,
        'seconds': seconds,
...
exports.decimalDegreesToDMS = decimalDegreesToDMS;
function getActivationFromDecimalDegrees(decimalDegrees) {
    var degreesFloat = parseFloat(decimalDegrees);
    ...
    var percentageThrough = degreesFloat / 360; // e.g. 182.3705 becomes 0.5065
    ...
    return {
        Gate: (exports.Gates.order[Math.floor(percentageThrough * 64)]),
...
exports.getActivationFromDecimalDegrees = getActivationFromDecimalDegrees;
...
exports.motorToThroat = motorToThroat;
