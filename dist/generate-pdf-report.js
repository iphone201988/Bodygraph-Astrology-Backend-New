...
const bodygraph = __importStar(require("./models/bodygraph"));
const page_text_json_1 = __importDefault(require("./strings/page-text.json"));
const channels_json_1 = __importDefault(require("./strings/channels.json"));
const strategies_json_1 = __importDefault(require("./strings/strategies.json"));
const signatures_json_1 = __importDefault(require("./strings/signatures.json"));
const not_self_themes_json_1 = __importDefault(require("./strings/not-self-themes.json"));
const determinations_json_1 = __importDefault(require("./strings/determinations.json"));
const environments_json_1 = __importDefault(require("./strings/environments.json"));
const motivations_json_1 = __importDefault(require("./strings/motivations.json"));
const views_json_1 = __importDefault(require("./strings/views.json"));
const cognitive_tones_json_1 = __importDefault(require("./strings/cognitive-tones.json"));
const environmental_tones_json_1 = __importDefault(require("./strings/environmental-tones.json"));
const transferences_json_1 = __importDefault(require("./strings/transferences.json"));
const distractions_json_1 = __importDefault(require("./strings/distractions.json"));
...
router.post("/", async function (req, res) {
    try {
        ...
        if (req.body.location == "") {
            ...
            console.log(res);
            timezone = find(res.latitude, res.longitude)[0];
        ...
        var bg = bodygraph.createBodygraph(req.body.name, date, req.body.location || "London, United Kingdom");
        // Print the bodygraph to the console
        console.log(bg);
        function getFormattedEnergyType(bg) {
        ...
        function getDetermination(bg) {
            let type = (bg.activations.Design.Sun.Tone < 4) ?
                determinations_json_1.default.leftVariableVariants[bg.activations.Design.Sun.Color] :
                determinations_json_1.default.rightVariableVariants[bg.activations.Design.Sun.Color];
            return `${type} ${determinations_json_1.default[bg.activations.Design.Sun.Color]}`;
        }
        function getEnvironment(bg) {
            let type = (bg.activations.Design.Sun.Tone < 4) ?
                environments_json_1.default.leftVariableVariants[bg.activations.Design.SouthNode.Color] :
                environments_json_1.default.rightVariableVariants[bg.activations.Design.SouthNode.Color];
            return `${type} ${environments_json_1.default[bg.activations.Design.SouthNode.Color]}`;
        }
        function getFormattedCircuitries(bg) {
            let circuitry = "";
        ...
        function getFormattedChannels(bg) {
            let formattedChannels = "";
            for (let i = 0; i < bg.channels.length; i++) {
                formattedChannels += `${channels_json_1.default.names[bg.channels[i]]} (${bg.channels[i]})`;
        ...
        var docDefinition = {
            content: [
                ...
                { text: page_text_json_1.default.page_2_about_this_hd_differentiation_guide, style: 'subheader', margin: [0, 0, 0, 13] },
                {
                    columns: [
                        { text: page_text_json_1.default.page_2_left_column },
                        { text: page_text_json_1.default.page_2_right_column, style: 'rightColumn', pageBreak: 'after' }
                    ]
                },
                ...
                { image: './images/page-3.png', width: 516, margin: [0, 0, 0, 42] },
                {
                    columns: [
                        {
                            text: [
                                { text: page_text_json_1.default.page_3_quick_breakdown, style: 'subheader', margin: [0, 0, 0, 13], lineHeight: .5 },
                                { text: ' \n', fontSize: 21 },
                                { text: page_text_json_1.default.page_3_left_column, margin: [0, 0, 19, 0] }
                            ]
                        },
                        { text: page_text_json_1.default.page_3_right_column, style: 'rightColumn', pageBreak: 'after' }
                    ]
                },
                // Page 4 --- Profile, Energy Type, Authority, Strategy, Definition, Signature etc
                {
                    ...
                    table: {
                        ...
                        body: [
                            ...
                            [{ text: 'INNER AUTHORITY:', style: 'greyTableLabel' }, bg.authority],
                            [{ text: 'OUTER AUTHORITY:', style: 'greyTableLabel' }, bg.outerAuthority],
                            [{ text: 'STRATEGY', style: 'greyTableLabel' }, strategies_json_1.default[bg.type]],
                            [{ text: 'DEFINITION:', style: 'greyTableLabel' }, bg.definition],
                            [{ text: 'SIGNATURE:', style: 'greyTableLabel' }, signatures_json_1.default[bg.type]],
                            [{ text: 'NOT SELF THEME:', style: 'greyTableLabel' }, not_self_themes_json_1.default.types[bg.type]],
                            ...
                            [{ text: 'MOTIVATION:', style: 'greyTableLabel' }, motivations_json_1.default[bg.activations.Personality.Sun.Color]],
                            [{ text: 'VIEW:', style: 'greyTableLabel' }, views_json_1.default[bg.activations.Personality.NorthNode.Color]],
                            [{ text: 'COGNITIVE TONE:', style: 'greyTableLabel' }, cognitive_tones_json_1.default[bg.activations.Design.Sun.Tone]],
                            [{ text: 'ENVIRONMENTAL TONE:', style: 'greyTableLabel' }, environmental_tones_json_1.default[bg.activations.Design.SouthNode.Tone]],
                            [{ text: 'TRANSFERENCE:', style: 'greyTableLabel' }, transferences_json_1.default[bg.activations.Personality.Sun.Color]],
                            [{ text: 'DISTRACTION:', style: 'greyTableLabel' }, distractions_json_1.default[bg.activations.Personality.NorthNode.Color]],
                    ...
                    layout: {
                        ...
                        fillColor: function (i, node) { return '#ededed'; }
                    } // End layout
                },
        ...
        // pdfDoc.end();
        var astroDetail = [
            { text: 'TYPE:', data: bg.type },
            { text: 'ENERGY TYPE:', data: getFormattedEnergyType(bg) },
            { text: 'STRATEGY:', data: strategies_json_1.default[bg.type] },
            { text: 'INNER AUTHORITY:', data: bg.authority },
            // { text: 'OUTER AUTHORITY:', data: bg.outerAuthority },
            { text: 'DEFINITION:', data: bg.definition },
            { text: 'PROFILE:', data: bg.profile },
            { text: 'DETERMINATION:', data: getDetermination(bg) },
            { text: 'COGNITIVE TONE:', data: cognitive_tones_json_1.default[bg.activations.Design.Sun.Tone] },
            { text: 'ENVIRONMENT:', data: getEnvironment(bg) },
            { text: 'VIEW/PERSPECTIVE:', data: views_json_1.default[bg.activations.Personality.NorthNode.Color] },
            { text: 'MOTIVATION:', data: motivations_json_1.default[bg.activations.Personality.Sun.Color] },
            { text: 'SIGNATURE:', data: signatures_json_1.default[bg.type] },
            { text: 'NOT SELF THEME:', data: not_self_themes_json_1.default.types[bg.type] },
            // { text: 'INCARNATION CROSS:', data: 'Right Angle Cross of Eden\nPersonality Sun (12) Personality Earth (11)\nDesign Sun (36) Design Earth (6)' },
            { text: 'ENVIRONMENTAL TONE:', data: environmental_tones_json_1.default[bg.activations.Design.SouthNode.Tone] },
            { text: 'TRANSFERENCE:', data: transferences_json_1.default[bg.activations.Personality.Sun.Color] },
            { text: 'DISTRACTION:', data: distractions_json_1.default[bg.activations.Personality.NorthNode.Color] },
            { text: 'CIRCUITRY:', data: getFormattedCircuitries(bg) },
            { text: 'VARIABLE:', data: bg.variable },
        ];
        bg.totalProcessingTime = new Date().getTime() - startTime;
        bg.astroDetail = astroDetail;
